...
const DEFAULT_PROGRESS = {
  ...
  levels: {
    ...
    9:{unlocked:false,completed:false,stars:0},
    10:{unlocked:false,completed:false,stars:0},
  ...
  try {
    ...
    if (s){
      ...
      if (loaded.levels){
        for (let i = 0; i <= 11; i++){
          if (loaded.levels[i]){
            merged.levels[i] = Object.assign(
              merged.levels[i] || {unlocked:false,completed:false,stars:0},
              loaded.levels[i]
            );
          }
        }
...
function playHeadshotSound(){
  ...
  setTimeout(()=>playTone(300,0.15,'triangle',0.12), 50);
}
...
function stopHorrorAmbient(){
  activeAmbientNodes.forEach(n=>{ try{ n.osc.stop(); }catch(e){} });
  activeAmbientNodes=[];
}
function startDynamicMusic(){
  ...
  calmOsc.connect(cf); cf.connect(calmGain); calmOsc.start();
...
function createGroundTexture(theme){
  return getTex('g_'+theme, ()=>{
    ...
    for (let i=0;i<800;i++){
      const x=Math.random()*512,y=Math.random()*512,r=3+Math.random()*25;
      const a=0.04+Math.random()*0.14, dark=Math.random()<0.55;
...
function buildLevelEnvironment(levelNum){
  ...
  const ground=new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({map:groundTex, color:horrorMode?0x808080:0xffffff, roughness:1})
  );
...
function createZombie(isBoss, type='melee'){
  ...
  const fs=size*scale;
  const torso=new THREE.Mesh(new THREE.CylinderGeometry(0.35*fs,0.32*fs,1.0*fs,8), uniform);
  torso.position.y=0.5*fs; torso.castShadow=true; g.add(torso);
  const head=new THREE.Mesh(new THREE.SphereGeometry(0.28*fs,12,10), skinMat);
  head.position.y=1.45*fs; head.castShadow=true; head.userData.isHead=true; g.add(head);
  const eL=new THREE.Mesh(new THREE.SphereGeometry(0.055*fs,6,6), glow);
  eL.position.set(-0.11*fs,1.48*fs,-0.24*fs); g.add(eL);
  const eR=new THREE.Mesh(new THREE.SphereGeometry(0.055*fs,6,6), glow);
  eR.position.set(0.11*fs,1.48*fs,-0.24*fs); g.add(eR);
  ...
  if (!isBoss){
    if (type==='shooter'){
      const rifle=new THREE.Mesh(new THREE.BoxGeometry(0.08,0.1,0.7), new THREE.MeshStandardMaterial({color:0x111111, metalness:0.9, roughness:0.3}));
      rifle.position.set(0.35*fs,0.9*fs,-0.35*fs); rifle.rotation.z=-0.15; g.add(rifle);
      ...
      bag.position.set(0,0.5*fs,0.35*fs); g.add(bag);
    } else if (type==='tank'){
      const armor=new THREE.Mesh(new THREE.BoxGeometry(0.9*fs,1.0*fs,0.4*fs), new THREE.MeshStandardMaterial({color:0x2a2a2a, metalness:0.7, roughness:0.5}));
      armor.position.set(0,0.55*fs,0.15*fs); g.add(armor);
    } else if (type==='bomber'){
      const core=new THREE.Mesh(new THREE.SphereGeometry(0.18*fs,10,10), new THREE.MeshBasicMaterial({color:0xff0000}));
      core.position.set(0,0.5*fs,-0.2*fs); g.add(core);
      const light=new THREE.PointLight(0xff0000, 1.5, 5);
      light.position.copy(core.position); g.add(light);
    } else if (type==='runner'){
      const band=new THREE.Mesh(new THREE.BoxGeometry(0.4*fs,0.08*fs,0.05*fs), new THREE.MeshStandardMaterial({color:0xcc2200}));
      band.position.set(0,1.6*fs,-0.2*fs); g.add(band);
    }
  } else {
    const hornMat=new THREE.MeshStandardMaterial({color:0xaa8866});
    const hl=new THREE.Mesh(new THREE.ConeGeometry(0.1*fs,0.5*fs,6), hornMat);
    hl.position.set(-0.2*fs,1.75*fs,0); hl.rotation.z=0.5; g.add(hl);
    const hr=new THREE.Mesh(new THREE.ConeGeometry(0.1*fs,0.5*fs,6), hornMat);
    hr.position.set(0.2*fs,1.75*fs,0); hr.rotation.z=-0.5; g.add(hr);
    const aura=new THREE.PointLight(0xff3300, 1.5, 8);
    aura.position.y=1.2*fs; aura.userData.isBossAura=true; g.add(aura);
  }
  g.position.y=0.9*fs;
  return g;
}
...
function spawnEnemy(isBoss=false, forcedType=null){
  ...
  e.userData={
    ...
    radius:isBoss?1.2:type==='tank'?0.9:0.5,
...
function markTutStep(id){
  const s=tutorialState?.steps.find(s=>s.id===id);
