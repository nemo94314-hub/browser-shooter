...
function playTone(freq,dur,type='square',vol=0.08){
  ...
  g.gain.setValueAtTime(vol*volume,audioCtx.currentTime);
...
function createWeather(theme){
  ...
  scene.add(points);
  weather={points,velocities,type,positions};
}
...
function setupPostProcessing(){
  if (typeof THREE.EffectComposer==='undefined' || isMobile) return;
  try {
    composer=new THREE.EffectComposer(renderer);
    // SSAO — если включено
    if (ssaoEnabled && THREE.SSAOPass){
    ...
    // Bloom
    if (bloomEnabled && THREE.UnrealBloomPass){
  ...
  } catch(e){ console.error('PostFX:', e); composer=null; }
}
function rebuildPostProcessing(){
  composer=null; bloomPass=null; ssaoPass=null;
  setupPostProcessing();
}
...
function createBloodBurst(pos,isHead){
  const count=isHead?22:12,color=isHead?0xbb0000:0x880000;
  for (let i=0;i<count;i++){ const v=new THREE.Vector3((Math.random()-0.5)*6,Math.random()*4+1,(Math.random()-0.5)*6); spawnParticle(pos,v,color,0.035+Math.random()*0.055,0.5+Math.random()*0.6,true); }
}
...
function updateParticles(delta){
  for (let i=particles.length-1;i>=0;i--){
    ...
    if (p.userData.gravity && p.position.y<0.03){ p.position.y=0.03; p.userData.velocity.y*=-0.3; p.userData.velocity.x*=0.7; p.userData.velocity.z*=0.7; }
    if (p.userData.spin){ p.rotation.x+=p.userData.spin.x*delta; p.rotation.y+=p.userData.spin.y*delta; p.rotation.z+=p.userData.spin.z*delta; }
...
function clearLevelEnvironment(){
  for (let i=scene.children.length-1;i>=0;i--){
    ...
    if (c===camera || c.isLight || c===weather?.points) continue;
    scene.remove(c);
  }
  obstacles=[];
}
function buildLevelEnvironment(levelNum){
  ...
  if (isTutorial){ skyColor=0x8ec8e8; fogColor=0x9ed4e8; fogNear=60; fogFar=160; ambientLevel=1.0; }
    ...
    skyColor=light[lvl.theme]||0x87a5c4; fogColor=skyColor; fogNear=50; fogFar=130; ambientLevel=0.7;
  }
...
function createWeapon(type){
  ...
  if (type==='pistol'){ const s=new THREE.Mesh(new THREE.BoxGeometry(0.05,0.09,0.28),metal); s.position.set(0,0.02,-0.15); weaponGroup.add(s); const g=new THREE.Mesh(new THREE.BoxGeometry(0.06,0.2,0.08),grip); g.position.set(0,-0.15,0.02); g.rotation.x=0.25; weaponGroup.add(g); weaponGroup.position.set(0.22,-0.22,-0.4); }
  else if (type==='rifle'){ const body=new THREE.Mesh(new THREE.BoxGeometry(0.055,0.075,0.5),metal); body.position.set(0,0,-0.2); weaponGroup.add(body); const barrel=new THREE.Mesh(new THREE.CylinderGeometry(0.014,0.014,0.3,12),metal); barrel.rotation.x=Math.PI/2; barrel.position.set(0,0.005,-0.55); weaponGroup.add(barrel); const handguard=new THREE.Mesh(new THREE.BoxGeometry(0.06,0.06,0.22),black); handguard.position.set(0,0,-0.4); weaponGroup.add(handguard); const mag=new THREE.Mesh(new THREE.BoxGeometry(0.03,0.15,0.05),metal); mag.position.set(0,-0.11,0.02); mag.rotation.x=-0.15; weaponGroup.add(mag); const g=new THREE.Mesh(new THREE.BoxGeometry(0.04,0.14,0.055),grip); g.position.set(0,-0.1,0.12); g.rotation.x=0.28; weaponGroup.add(g); const stock=new THREE.Mesh(new THREE.BoxGeometry(0.04,0.09,0.18),black); stock.position.set(0,0.005,0.24); weaponGroup.add(stock); weaponGroup.position.set(0.28,-0.26,-0.5); }
  else if (type==='shotgun'){ const body=new THREE.Mesh(new THREE.BoxGeometry(0.07,0.085,0.45),metal); body.position.set(0,0,-0.15); weaponGroup.add(body); const barrel=new THREE.Mesh(new THREE.CylinderGeometry(0.022,0.022,0.4,12),metal); barrel.rotation.x=Math.PI/2; barrel.position.set(0,0.01,-0.5); weaponGroup.add(barrel); const pump=new THREE.Mesh(new THREE.BoxGeometry(0.07,0.05,0.14),wood); pump.position.set(0,-0.045,-0.36); weaponGroup.add(pump); const g=new THREE.Mesh(new THREE.BoxGeometry(0.045,0.15,0.06),wood); g.position.set(0,-0.11,0.13); g.rotation.x=0.25; weaponGroup.add(g); const stock=new THREE.Mesh(new THREE.BoxGeometry(0.045,0.1,0.2),wood); stock.position.set(0,0,0.26); weaponGroup.add(stock); weaponGroup.position.set(0.3,-0.28,-0.5); }
  else if (type==='sniper'){ const body=new THREE.Mesh(new THREE.BoxGeometry(0.055,0.075,0.6),metal); body.position.set(0,0,-0.25); weaponGroup.add(body); const barrel=new THREE.Mesh(new THREE.CylinderGeometry(0.014,0.012,0.5,12),metal); barrel.rotation.x=Math.PI/2; barrel.position.set(0,0.005,-0.8); weaponGroup.add(barrel); const scope=new THREE.Mesh(new THREE.CylinderGeometry(0.032,0.032,0.28,14),rail); scope.rotation.x=Math.PI/2; scope.position.set(0,0.075,-0.2); weaponGroup.add(scope); const lensF=new THREE.Mesh(new THREE.CircleGeometry(0.03,14),glass); lensF.position.set(0,0.075,-0.342); weaponGroup.add(lensF); const lensR=new THREE.Mesh(new THREE.CircleGeometry(0.03,14),glass); lensR.position.set(0,0.075,-0.058); lensR.rotation.y=Math.PI; weaponGroup.add(lensR); const g=new THREE.Mesh(new THREE.BoxGeometry(0.04,0.15,0.06),grip); g.position.set(0,-0.1,0.05); g.rotation.x=0.28; weaponGroup.add(g); const stock=new THREE.Mesh(new THREE.BoxGeometry(0.045,0.11,0.22),black); stock.position.set(0,0.01,0.2); weaponGroup.add(stock); weaponGroup.position.set(0.3,-0.26,-0.55); }
  else if (type==='dualPistols'){ [-0.16,0.16].forEach(dx=>{ const g=new THREE.Group(); const s=new THREE.Mesh(new THREE.BoxGeometry(0.05,0.09,0.24),metal); s.position.set(0,0.02,-0.12); g.add(s); const grip2=new THREE.Mesh(new THREE.BoxGeometry(0.06,0.18,0.06),grip); grip2.position.set(0,-0.13,0.06); grip2.rotation.x=0.22; g.add(grip2); g.position.set(dx,0,0); weaponGroup.add(g); }); weaponGroup.position.set(0,-0.25,-0.5); }
  else if (type==='flamethrower'){ const body=new THREE.Mesh(new THREE.BoxGeometry(0.09,0.09,0.4),metal); body.position.set(0,0,-0.15); weaponGroup.add(body); const tank=new THREE.Mesh(new THREE.CylinderGeometry(0.05,0.05,0.35,14),new THREE.MeshStandardMaterial({color:0xcc2200,metalness:0.85,roughness:0.35})); tank.position.set(-0.09,-0.05,-0.1); tank.rotation.z=Math.PI/2; weaponGroup.add(tank); const nozzle=new THREE.Mesh(new THREE.CylinderGeometry(0.018,0.025,0.28,10),metal); nozzle.rotation.x=Math.PI/2; nozzle.position.set(0,0.01,-0.44); weaponGroup.add(nozzle); const g=new THREE.Mesh(new THREE.BoxGeometry(0.04,0.15,0.06),grip); g.position.set(0,-0.1,0.1); g.rotation.x=0.25; weaponGroup.add(g); weaponGroup.position.set(0.28,-0.28,-0.5); }
...
function checkBossPhase(){ if(!currentBoss || !currentBoss.userData) return; const ratio=currentBoss.userData.health/currentBoss.userData.maxHealth; const ud=currentBoss.userData; let np=1; if(ratio<=0.33) np=3; else if(ratio<=0.66) np=2; if(np!==ud.phase){ ud.phase=np; bossPhase=np; playBossRoar(); shakeAmount=1; currentBoss.traverse(ch=>{ if(ch.isMesh && ch.material && ch.material.color){ ch.material=ch.material.clone(); if(np===2) ch.material.color.setHex(0x8a3a1a); else if(np===3) ch.material.color.setHex(0xaa0000); } }); showToast(np===2?'⚡ ФАЗА 2!':'💀 ФАЗА 3!',2500); } }
...
function enemyShoot(e,target){ const b=new THREE.Mesh(new THREE.SphereGeometry(0.1,6,6),new THREE.MeshBasicMaterial({color:0xff3300})); b.position.copy(e.position).add(new THREE.Vector3(0,e.userData.isBoss?2:1.2,0)); const dir=new THREE.Vector3().subVectors(target,b.position).normalize(); b.userData={velocity:dir.multiplyScalar(25),life:3,damage:e.userData.isBoss?15:10}; scene.add(b);enemyBullets.push(b); playShootSoundEnhanced(); }
function throwGrenade(e,target){ const g=new THREE.Mesh(new THREE.SphereGeometry(0.15,8,8),new THREE.MeshStandardMaterial({color:0x2a5a2a,metalness:0.8,roughness:0.2})); g.position.copy(e.position).add(new THREE.Vector3(0,e.userData.isBoss?2:1.2,0)); const dir=new THREE.Vector3().subVectors(target,g.position).normalize(); g.userData={velocity:dir.multiplyScalar(12).add(new THREE.Vector3(0,5,0)),life:2.5,damage:40,radius:6,isPlayer:false}; scene.add(g);grenades.push(g); }
function throwPlayerGrenade(){ if (!isGameActive || playerGrenades<=0) return; playerGrenades--; const g=new THREE.Mesh(new THREE.SphereGeometry(0.15,8,8),new THREE.MeshStandardMaterial({color:0x2a5a2a,metalness:0.8,roughness:0.2})); const fwd=new THREE.Vector3(0,0,-1).applyQuaternion(camera.quaternion); g.position.copy(camera.position).add(fwd.clone().multiplyScalar(0.5)); g.position.y-=0.2; const v=fwd.clone().multiplyScalar(15); v.y+=6; g.userData={velocity:v,life:2.5,damage:80,radius:8,isPlayer:true}; scene.add(g);grenades.push(g); playClickSound(300,0.05); updateHUD(); if (tutorialState) markTutStep('grenade'); }
function updateBullets(delta){
  for (let i=enemyBullets.length-1;i>=0;i--){ const b=enemyBullets[i]; b.userData.life-=delta; b.position.add(b.userData.velocity.clone().multiplyScalar(delta)); if (b.userData.life<=0 || b.position.length()>100){ scene.remove(b); enemyBullets.splice(i,1); continue; } if (b.position.distanceTo(camera.position)<1){ takeDamage(b.userData.damage*(survivalMode?survivalMode.enemyDamage:1)); scene.remove(b); enemyBullets.splice(i,1); } }
  for (let i=grenades.length-1;i>=0;i--){ const g=grenades[i]; g.userData.life-=delta; g.userData.velocity.y-=GRAVITY*delta; g.position.add(g.userData.velocity.clone().multiplyScalar(delta)); if (g.position.y<0.2){ g.userData.velocity.y*=-0.5; g.position.y=0.2; } if (g.userData.life<=0){ explodeGrenade(g.position,g.userData.damage,g.userData.radius,g.userData.isPlayer); scene.remove(g); grenades.splice(i,1); } }
}
function explodeGrenade(pos,damage,radius,isPlayer){
  ...
  let killed=0;
  enemies.forEach(e=>{ if (e.userData.isTarget) return; const d=pos.distanceTo(e.position); if (d<radius){ const alive=e.userData.health>0; e.userData.health-=damage*(1-d/radius); if (alive && e.userData.health<=0) killed++; } });
...
function takeDamage(amount){ if (!isGameActive) return; health-=amount; sessionStats.damageTaken+=amount; if (tutorialState) tutorialState.damageTaken+=amount; shakeAmount=Math.min(1,shakeAmount+0.3); playHurtSoundEnhanced(); const vig=document.getElementById('damageVignette'); if (vig){ vig.style.opacity='1'; setTimeout(()=>vig.style.opacity='0',200); } updateHUD(); if (health<=0){ health=0; gameOver(); } }
...
function createLootCrate(pos,weaponKey){ const c=new THREE.Mesh(new THREE.BoxGeometry(0.6,0.6,0.6),new THREE.MeshStandardMaterial({color:0x8a6a2a,metalness:0.5,roughness:0.5})); c.position.copy(pos); c.position.y=0.3; c.userData={weapon:weaponKey}; scene.add(c); lootCrates.push(c); }
function updateLoot(delta){ nearLootCrate=null; lootCrates.forEach(c=>{ c.rotation.y+=delta*2; if (c.position.distanceTo(camera.position)<2.5) nearLootCrate=c; }); const prompt=document.getElementById('interactPrompt'); if (prompt) prompt.style.display=nearLootCrate?'block':'none'; }
function pickupLoot(){ if (!nearLootCrate) return; const w=nearLootCrate.userData.weapon; if (WEAPONS[w]){ WEAPONS[w].ammo=WEAPONS[w].maxAmmo; playPickupSound(); addScore(50); } const wasTut=nearLootCrate.userData.isTutorialCrate; scene.remove(nearLootCrate); lootCrates=lootCrates.filter(c=>c!==nearLootCrate); nearLootCrate=null; updateHUD(); if (wasTut && tutorialState) markTutStep('pickup'); }
function createCorpse(enemy){ const c=enemy.clone(); c.rotation.x=Math.PI/2; c.position.y=0.1; c.traverse(ch=>{ if (ch.isMesh) ch.material=ch.material.clone(); }); scene.add(c); corpses.push(c); setTimeout(()=>{ scene.remove(c); corpses=corpses.filter(x=>x!==c); },10000); const stain=new THREE.Mesh(new THREE.CircleGeometry(0.8,8),new THREE.MeshBasicMaterial({color:0x6a0000,transparent:true,opacity:0.7})); stain.rotation.x=-Math.PI/2; stain.position.set(enemy.position.x,0.02,enemy.position.z); scene.add(stain); bloodStains.push(stain); setTimeout(()=>{ scene.remove(stain); bloodStains=bloodStains.filter(s=>s!==stain); },15000); }
...
function showHitMarker(isHead){ const hm=document.getElementById('hitMarker'); if (!hm) return; hm.textContent='✖'; hm.style.color=isHead?'#ffcc00':'#ffffff'; hm.style.fontSize=isHead?'42px':'30px'; hm.classList.add('show'); clearTimeout(hm._t); hm._t=setTimeout(()=>hm.classList.remove('show'),120); }
function reload(){ if (reloading) return; const w=WEAPONS[currentWeapon]; if (w.ammo===w.maxAmmo) return; reloading=true; playReloadSoundEnhanced(); const t=w.reload*getReloadMultiplier(); showToast('Перезарядка...',t); setTimeout(()=>{ w.ammo=w.maxAmmo; reloading=false; updateHUD(); },t); if (tutorialState) markTutStep('reload'); }
function switchWeapon(key){ if (!WEAPONS[key]) return; if (!PROGRESS.ownedWeapons.includes(key)) return; currentWeapon=key; createWeapon(key); updateHUD(); }
...
function renderDailyReward(){
  ...
  if (!grid) return;
  let html='';
  const currentStreak=s.streak;
  for (let i=0;i<7;i++){
    const dayNum=i+1;
    const claimed=dayNum<=currentStreak;
    const isToday=s.canClaim && dayNum===currentStreak+1;
...
function initUI(){
  ...
  document.getElementById('dailyBtn')?.addEventListener('click',()=>{ renderDailyReward(); showScreen('dailyModal'); });
  document.getElementById('dailyClaimBtn')?.addEventListener('click',claimDailyReward);
  document.getElementById('dailyCloseBtn')?.addEventListener('click',()=>showScreen('menu'));
  ...
  document.getElementById('horrorToggle')?.addEventListener('change',e=>{ horrorMode=e.target.checked; try{ localStorage.setItem('zombieshoot_horror',horrorMode); }catch(e){} });
  document.getElementById('bloomToggle')?.addEventListener('change',e=>{ bloomEnabled=e.target.checked; try{ localStorage.setItem('zombieshoot_bloom',bloomEnabled); }catch(e){} rebuildPostProcessing(); });
  document.getElementById('ssaoToggle')?.addEventListener('change',e=>{ ssaoEnabled=e.target.checked; try{ localStorage.setItem('zombieshoot_ssao',ssaoEnabled); }catch(e){} rebuildPostProcessing(); });
  ...
  if (document.getElementById('bloomToggle')) document.getElementById('bloomToggle').checked=bloomEnabled;
  if (document.getElementById('ssaoToggle')) document.getElementById('ssaoToggle').checked=ssaoEnabled;
}
function showScreen(id){ document.querySelectorAll('.screen').forEach(s=>s.style.display='none'); if (id) document.getElementById(id).style.display='flex'; renderer.domElement.style.display='none'; }
...
function handleWeapon(key){ if (PROGRESS.ownedWeapons.includes(key)){ switchWeapon(key); playClickSound(500,0.05); } else { const w=WEAPONS[key]; if (PROGRESS.coins>=w.cost){ PROGRESS.coins-=w.cost; PROGRESS.ownedWeapons.push(key); saveProgress(); playBuySound(); switchWeapon(key); } else playErrorSound(); } updateCoinsDisplay(); renderShop('weapons'); checkAchConditions(); }
function handleSkin(key){ if (PROGRESS.ownedSkins.includes(key)){ PROGRESS.currentSkin=key; saveProgress(); playClickSound(500,0.05); } else { const sk=SKINS[key]; if (PROGRESS.coins>=sk.cost){ PROGRESS.coins-=sk.cost; PROGRESS.ownedSkins.push(key); PROGRESS.currentSkin=key; saveProgress(); playBuySound(); } else playErrorSound(); } updateCoinsDisplay(); renderShop('skins'); checkAchConditions(); }
...
function showAchToast(ach){ let el=document.getElementById('achToast'); if (!el){ el=document.createElement('div'); el.id='achToast'; document.body.appendChild(el); } el.innerHTML=`<div class="toast-icon">${ach.icon}</div><div class="toast-content"><div class="toast-label">🏆 ДОСТИЖЕНИЕ</div><div class="toast-name">${ach.name}</div><div class="toast-desc">${ach.desc}</div></div>`; el.classList.add('show'); clearTimeout(el._t); el._t=setTimeout(()=>el.classList.remove('show'),4000); }
function updateAchCount(){ const n=Object.keys(PROGRESS.achievements||{}).length; const total=Object.keys(ACHIEVEMENTS).length; const a=document.getElementById('achProgress'); const b=document.getElementById('achCount'); if (a) a.textContent=`${n}/${total}`; if (b) b.textContent=`${n}/${total}`; }
function renderAchievements(){ const grid=document.getElementById('achGrid'); if (!grid) return; let html=''; for (const id in ACHIEVEMENTS){ const a=ACHIEVEMENTS[id]; const un=hasAch(id); html+=`<div class="ach-card ${un?'unlocked':'locked'}"><div class="ach-icon">${a.icon}</div><div class="ach-info"><div class="ach-name">${a.name}</div><div class="ach-desc">${a.desc}</div></div>${!un?'<div class="ach-lock">🔒</div>':''}</div>`; } grid.innerHTML=html; updateAchCount(); }
...
function updateTutorialHUD(){ const el=document.getElementById('tutorialHUD'); if (!el || !tutorialState) return; const list=tutorialState.steps.map(s=>`<div class="tut-step ${s.done?'done':''}"><span class="tut-check">${s.done?'✅':'⬜'}</span><span class="tut-text">${s.text}</span></div>`).join(''); el.innerHTML=`<div class="tut-title">🎓 ТРЕНИРОВКА</div>${list}`; }
